...
def test_minimum_plan_stays_within_available_time():
    ...
    assert sum(item.estimated_minutes for item in plan.tasks) <= 90
    assert {item.category for item in plan.tasks} >= {"math", "english", "computer"}
    assert not any(item.category == "ai_project" for item in plan.tasks)


def test_ample_plan_can_include_ai_project():
    plan = build_rule_plan(MorningCheckIn(energy="ample", available_minutes=300, day_type="normal"))
    assert any(item.category == "ai_project" for item in plan.tasks)


def test_early_day_shortens_sleep_target():
    plan = build_rule_plan(MorningCheckIn(energy="normal", available_minutes=180, day_type="early_class"))
    sleep = next(item for item in plan.tasks if item.category == "sleep")
    assert "07:30" in sleep.completion_criteria
